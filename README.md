...
- [Sudarshan](https://github.com/Suraj1235/sudarshan-superharness) — Provider-neutral harness that drives an LLM API or a bridged agent CLI from an idea, PRD, or spec to verification-gated completion. Explainable cost estimates before the run, hard budget ceilings, immutable plans, and atomic checkpoints that resume after process kills or rate limits. Zero-dependency Python, MIT.
