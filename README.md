...
- [llmtrim](https://github.com/fkiene/llmtrim) — Open-source proxy, MCP server, CLI, and library for AI coding agents that compresses prompts, tool outputs, and replies to cut token cost. Quality-gated so it never raises the bill, and the cached prefix is never rewritten. -31% input and -74% output tokens across 112 live A/B cases. AGPL-3.0.
