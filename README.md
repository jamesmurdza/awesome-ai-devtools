...
- [mcp-config-lint](https://github.com/basilalshukaili/mcp-config-lint) — CLI tool that lints, security-checks, and estimates token costs for Claude Desktop MCP configs (`claude_desktop_config.json`). Detects credential exposure, path traversal risks, and misconfigured servers. CI-friendly exit codes, JSON output, zero dependencies. `pip install mcp-config-lint`
