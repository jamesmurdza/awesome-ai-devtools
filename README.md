...
- [reflint](https://github.com/hyuga611/reflint) — Reference-integrity linter for agent config files (AGENTS.md, llms.txt, CLAUDE.md). Fails the PR when a referenced command, npm script, or path no longer resolves, including markdown link targets inside llms.txt. Language-agnostic, ships as a GitHub Action with inline PR annotations and a textlint rule adapter. Zero dependencies, MIT.
- [skills-lint](https://github.com/hyuga611/skills-lint) — Linter for Anthropic Agent Skills (SKILL.md). Validates frontmatter schema, checks that referenced files exist, and detects collisions between skills — duplicate name values, or descriptions similar enough that the agent fires the wrong one. Zero dependencies, MIT.
- [carrylint](https://github.com/hyuga611/carrylint) — Runtime-portability linter for SKILL.md, AGENTS.md, and slash commands. Flags assumptions that break on another machine or model: absolute paths, undeclared external CLIs, unresolved placeholders, hardcoded model ids. Static analysis only, no API key at runtime. Zero dependencies, MIT.
