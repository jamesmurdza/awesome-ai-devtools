...
- [Baselane](https://github.com/baselane-sh/baselane) — Git-native package manager for AI coding-agent configs. Audits a repo and applies versioned workflow packs rendered into AGENTS.md, CLAUDE.md, Copilot, and Gemini adapters with merge-safe managed regions, and distributes config by PR across repos. State is declared in a committed [harness.json](https://github.com/baselane-sh/harness.json) manifest with exact version pins and a published JSON Schema. Zero runtime dependencies, Apache-2.0.
