...
- [CodeAgentSwarm](https://www.codeagentswarm.com) — Desktop app (macOS, Windows) for running multiple Claude Code, Codex CLI and Gemini CLI agents in parallel supervised terminals. Includes desktop notifications, cross-session conversation history, per-terminal file diffs, and an MCP-connected kanban board.
