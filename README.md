...
- [Operator](https://github.com/iishyfishyy/operator-oss) — Open-source, local-first web control room that runs many Claude Code and Codex sessions in parallel across multiple projects from one screen. Each task is its own agent session in an isolated git worktree, with diff review and merge built in; uses your existing agent login, no API key. Apache-2.0.
