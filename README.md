...
- [Ordewell](https://github.com/ordewell/ordewell) — Plan-first orchestrator for coding agents: a read-only planner turns one goal into an ordered, editable plan of tasks, each assigned its own runner (Claude Code, Codex, OpenCode), model, thinking effort and mode. Each task runs as one agent session and is marked done only when its own completion marker appears. Apache-2.0.
