...
- [BitRouter](https://bitrouter.ai) — Agent-native LLM router that optimizes your agent with every run — zero harness changes, with every model call reliable, traceable, secure, and cost-effective. Routes across OpenAI, Anthropic, Google, OpenRouter, Bedrock, GitHub Copilot, Codex, and OpenCode Zen/Go through one local endpoint; integrates Claude Code, Codex, and Gemini CLI via ACP, with an MCP gateway, cross-protocol routing, guardrails, virtual API keys, and multi-account failover. Built in Rust, Apache-2.0.
