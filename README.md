...
- [Podiom](https://github.com/Podiom/Podiom) — Go-based control plane that orchestrates Claude Code and Codex CLI agents: durable named agents, chat sessions that replay onto a fresh backing CLI session on any provider/profile switch, a shared project ledger, and an embedded scheduler for recurring work. Single Go binary with an embedded Svelte web UI. MIT licensed.
