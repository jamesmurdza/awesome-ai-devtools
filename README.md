...
- [forge-dev-configs](https://github.com/FoundryNet/forge-dev-configs) — Drop-in AI coding-tool configs for industrial equipment telemetry (CNC, robots, PLCs, vehicles, HVAC). Generates `.cursor/rules/*.mdc`, `.cursorrules`, `.windsurfrules`, `AGENTS.md`, `copilot-instructions.md`, and a VS Code fragment from the MIT-licensed FoundryNet canonical schema — 366 field names verified against the schema at build time, so a rules file meant to stop hallucinated field names cannot itself contain them. MIT.
