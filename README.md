...
- [faf-cli](https://github.com/Wolfe-Jam/faf-cli) — CLI that authors a repository's AI context as one typed source file (project.faf) and projects it to CLAUDE.md, AGENTS.md, GEMINI.md, and .cursorrules, re-deriving them from the detected stack as the code changes. The .faf format is IANA-registered (application/vnd.faf+yaml); a deterministic score rates context completeness.
