...
- [intentic](https://intentic.dev/) — Browser workspace for a fleet of coding agents, each in a persistent Docker sandbox on hardware you own with a git worktree of its own. Runs continue after the browser closes and reopen from any device, including a phone. Plan mode by default, per-hunk diff review, credential capabilities injected per turn, and automations that wake an agent on a schedule, webhook or event. Claude Code, Codex, Grok, Kimi Code and Gemini on your own subscriptions. MIT, TypeScript.
