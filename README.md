...
- [codesight](https://github.com/Houseofmvps/codesight) — CLI token optimizer and AI context generator.
...
- [Shep](https://github.com/shep-ai/cli) — Multi-session SDLC control center that orchestrates AI coding agents
- [SuperCompress](https://supercompress.vercel.app) — Learned prompt compression that reduces LLM token costs by ~65% using a ~5K-param CPU policy (~60ms latency). Ideal for keeping agent context lean. Open-source (MIT). (Claude Code, Cursor CLI, Gemini) for autonomous feature development with configurable approval gates and a live web dashboard.
