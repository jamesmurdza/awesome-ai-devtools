...
- [Uni-CLI](https://github.com/olo-dot-io/Uni-CLI) — Self-repairing CLI catalog that exposes web, desktop apps, Electron apps, and bridge CLIs as deterministic commands for AI agents. Declarative YAML adapters with structured error envelopes (`adapter_path`, `step`, `suggestion`) let agents fix failing adapters and retry. Optional MCP server (stdio + Streamable HTTP). Live catalog size and per-call token budget tracked in the repo's [README](https://github.com/olo-dot-io/Uni-CLI#readme) and [`docs/BENCHMARK.md`](https://github.com/olo-dot-io/Uni-CLI/blob/main/docs/BENCHMARK.md).
