...
- [TUICommander](https://github.com/sstraus/tuicommander) — Cross-platform desktop workspace that runs up to 50 AI coding agents in parallel, each in its own git worktree. Detects agent state (working, awaiting input, rate limited) from terminal output, and adds diffs, GitHub PR and issue panels, a code editor, an MCP proxy hub and a companion PWA for answering agent prompts from a phone. Rust and Tauri, Apache-2.0.
