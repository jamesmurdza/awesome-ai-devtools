...
- [Tack](https://github.com/Dkibkalo/tack) — Script tag for annotating any rendered web page and handing a coding agent context it can resolve. Click an element to leave a note, or rewrite its text in place so the export carries an exact replacement rather than a description. Each note records element text, nearest heading and CSS selector, plus a stated protocol telling the agent which anchor to trust. Framework-agnostic (reads rendered DOM only), 11KB, zero dependencies, no network calls, MIT.
