...
- [slopless](https://github.com/seochecks-ai/slopless) — Deterministic textlint preset and CLI to flag AI-generated and padded English prose without calling an LLM. Catches the LLM tells (hollow framing, fake "not X but Y" contrasts, hedging, em-dash overuse, vacuous closers) in coding-agent and writing-agent output. Zero-config (`npx slopless`), runs in CI, emits JSON findings. MIT.
