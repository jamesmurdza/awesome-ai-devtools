...
- [DevIntern](https://github.com/getdevintern/devintern) — Turns task-tracker tickets (Jira, Linear, Trello, Asana, Azure DevOps, GitHub Issues, or markdown files) into self-reviewed pull requests using your choice of coding agent (Claude Code, Codex, Cursor, OpenCode) and model. A feasibility gate flags vague tickets back to the tracker with questions, and an optional unattended mode schedules ticket pickup and turns PR review comments into commits. Source-available (FSL), free for interactive use, runs on your machines with your own keys.
