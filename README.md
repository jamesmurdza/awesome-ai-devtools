...
- [claude-session-sync](https://github.com/defi0x1/claude-session-sync) — Syncs Claude Code session transcripts between machines through a private Git repository you own, so a session started on one machine can be resumed on another. Opt-in per session, optional age encryption, and a rewrite of the project path so the same session resolves under a different checkout location. POSIX shell; Git is the only runtime dependency, with python3 or jq used at setup to edit settings.json. macOS, Linux, and Windows via Git Bash. MIT.
