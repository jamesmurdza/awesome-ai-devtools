...
- [zeroshot](https://github.com/covibes/zeroshot) — Open-source CLI that runs a planner, an implementer, and independent validators in isolated environments (local, git worktree, or Docker), looping until a change is verified or rejected with a reproducible failure. Works with Claude, Codex, Gemini, and OpenCode CLIs; pulls issues from GitHub, GitLab, Jira, and Azure DevOps. Node, MIT licensed.
