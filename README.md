...
- [Fletch](https://fletch.sh/?utm_source=dir-awesome-ai-devtools) — Native macOS app that runs Claude Code, Codex, Cursor Agent, and OpenCode in sealed workspaces: each agent gets its own clone with its own writable .git inside a macOS Seatbelt or Docker sandbox, so the working copy is never writable by an agent. A symbol and call-graph index of the repo is served to every agent over MCP, built in a throwaway mirror so nothing is written to the source repository. Workflows advance on verifiable conditions (tests passing, commits landing, human approval) with engine-enforced budgets for turns, tokens, and wall-clock time. Push and pull requests run host-side, so credentials never enter a sandbox. AGPL-3.0.
