...
- [Aeon](https://github.com/aeonfun/aeon) — Autonomous agent framework that runs entirely inside GitHub Actions. Cron-scheduled or repo-triggered Markdown "skills" dispatch to six coding-agent harnesses (Claude Code, Codex, Grok, Pi, Vibe, Kimi) behind a single contract, with automatic dispatch, merge, and recovery. Self-heals by detecting failing skills and repairing them via PR, and spawns a fleet of isolated clones. State lives in the git repo; ships an MCP server exposing every skill as a tool. MIT.
