...
- [AgentBox](https://github.com/madarco/agentbox) — Run multiple coding agents (Claude Code, Codex, OpenCode) in parallel, each teleported into its own sandboxed VM (local Docker, self-hosted, or cloud: Hetzner, Daytona, Vercel, E2B, DigitalOcean). Sub-1s checkpoint startup, per-box browser/VS Code/persistent shells, git credentials kept on the host. Native macOS menu-bar app. MIT.
