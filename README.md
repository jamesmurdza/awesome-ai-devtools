...
- [agent-harness](https://github.com/uzysjung/uzys-agent-harness) — One-command installer and curator that sets up vetted, tech-stack-matched AI-coding skills, plugins, rules, and hooks across Claude Code (first-class), Codex, OpenCode, and Antigravity. Trust-tier curation (≥1000★ + active maintenance + Docker install-verification, 49/58 assets green) with monthly star-drift monitoring; project-scoped by default. `npx @uzysjung/agent-harness`. MIT.
