...
- [Shep](https://github.com/shep-ai/cli) — Multi-session SDLC control center
- [Hashgraph Online (HOL)](https://hol.org) — Universal agent registry with Universal Agent IDs (UAIDs) for agent identity, trustless P2P communication, and cross-protocol bridging (A2A, ERC-8004, Virtuals, x402). 187K+ verified agents. TypeScript, Go, Python SDKs.
 that orchestrates AI coding agents (Claude Code, Cursor CLI, Gemini) for autonomous feature development with configurable approval gates and a live web dashboard.
