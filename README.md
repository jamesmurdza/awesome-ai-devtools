...
- [GraphCoding](https://github.com/mosabsayyed/graphcoding) — In-repo knowledge graph that AI agents code against: planned files and scheduled deletions are first-class graph data, and a pre-commit/CI drift gate blocks commits until code matches the declared design. Zero-dependency Python CLI (scan, plan, sync, drift, blast-radius queries), sorted-JSONL storage diffable in PRs. Ships a Claude Code skill and Cursor rules. MIT.
