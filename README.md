...
- [Dely](https://github.com/hieuphung97/dely) — Control protocol that dispatches implementation and review to separate worker sessions on different harnesses, so nothing reviews its own diff. Two human gates only: approve the design contract, merge the prepared exact-HEAD PR. Works with seven CLIs today, including Claude Code, Codex CLI, Cursor Agent CLI, Grok Build, and GitHub Copilot CLI, with more added as they ship. Runs on Orca as the execution plane. MIT.
