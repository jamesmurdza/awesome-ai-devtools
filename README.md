...
- [Dahrk](https://github.com/dahrkai/dahrk-node) — Turns a Linear issue into a multi-stage agent workflow on a node you run. Each stage gets its own runtime (Claude Code, Codex, Pi), model, prompt, and git worktree, with approval gates you control. Plain TypeScript sequences the stages. Apache-2.0 node, hosted hub.
