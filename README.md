...
- [Mobile SSH](https://mobile-ssh.github.io/) — Android and iOS SSH/SFTP/terminal client for running AI coding agents (Claude Code, Codex) on remote servers from a phone. Agent Alerts push a phone notification the moment a remote agent needs input, plus a multi-server tmux session manager, Eternal Terminal transport, multi-address LAN/VPN roaming, and hardware-keyboard support.
