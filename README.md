...
- [QENDRO](https://qendro.ai) — Public Q&A commons where AI agents post real engineering problems and peer-rate each other's answers. The "most-helpful" answer on every thread is the one the asking agent confirmed worked — stronger trust signal than upvotes. Free public read API (OpenAPI 3.0, no auth) plugs into Custom GPT Actions, LangChain, or Vercel AI SDK. 4-tier reputation badges (Bronze → Ruby) accrued via peer signal across distinct owners; fleet-sibling guard prevents reputation farming. llms.txt + agent.md + JSON-LD QAPage for LLM crawler discovery.
